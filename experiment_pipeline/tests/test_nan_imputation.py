"""
Unit tests for core/nan_imputation.py — short-gap NaN filling.

Tests:
  - Short gap (<=5 min) → forward fill
  - Longer gap (>5 min) → stays NaN
  - No NaN → unchanged
...
# ============================================================================
# Longer gap tests (remain NaN)
# ============================================================================

class TestLongerGapRemainsNaN:
    """Gaps > 5 minutes should remain NaN."""

    def test_6min_gap_stays_nan(self):
        """A 6-minute gap should NOT be filled."""
        n = 20
        ts = _make_ts(n)
        vals = np.full(n, 100.0)
        vals[8:14] = np.nan  # 6-minute gap

        df = pd.DataFrame({'timestamp': ts, 'w1': vals, 'w2': vals.copy(), 'w3': vals.copy()})
        result = impute_nan_gaps(df, phase_cols=['w1'])

        assert result['w1'].isna().sum() == 6

    def test_10min_gap_stays_nan(self):
        """A 10-minute gap should remain NaN."""
        n = 30
        ts = _make_ts(n)
        vals = np.full(n, 100.0)
        vals[10:20] = np.nan  # 10-minute gap

        df = pd.DataFrame({'timestamp': ts, 'w1': vals, 'w2': vals.copy(), 'w3': vals.copy()})
        result = impute_nan_gaps(df, phase_cols=['w1'])

        assert result['w1'].isna().sum() == 10

    def test_60min_gap_stays_nan(self):
        """A 60-minute gap should remain NaN."""
        n = 80
        ts = _make_ts(n)
        vals = np.full(n, 500.0)
        vals[10:70] = np.nan  # 60-minute gap

        df = pd.DataFrame({'timestamp': ts, 'w1': vals, 'w2': vals.copy(), 'w3': vals.copy()})
        result = impute_nan_gaps(df, phase_cols=['w1'])

        assert result['w1'].isna().sum() == 60
...
class TestMixedGaps:
    ...
    def test_mixed_short_and_long(self):
        ...
        # Short gap: 3 min (indices 10-12) — should be filled
        vals[10:13] = np.nan
        # Medium gap: 20 min (indices 50-69) — should remain NaN
        vals[50:70] = np.nan
        # Long gap: 80 min (indices 100-179) — should remain NaN
        vals[100:180] = np.nan
        ...
        # Medium gap should remain NaN
        assert result['w1'].iloc[50:70].isna().sum() == 20
...
class TestMultiPhase:
    ...
    def test_different_phases_different_gaps(self):
        ...
        w1[5:8] = np.nan    # 3 min — short, filled
        w2[10:20] = np.nan  # 10 min — remains NaN
        # w3 has no NaN
        ...
        assert result['w1'].isna().sum() == 0
        assert result['w2'].isna().sum() == 10  # 10-min gap stays NaN
