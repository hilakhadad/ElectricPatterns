...
def split_by_day_night_from_path(filepath, additional_points=None, chunksize=10000):
        ...
        while segment_start <= chunk['timestamp'].max():
            ...
            if not segment.empty:
                ...
                if additional_points is not None:
                    points_in_segment = additional_points[
                        (additional_points['start'] >= segment_start) &
                        (additional_points['start'] < segment_end)
                    ]
...
def plot_combined_phases_interactive(df, phases, filepath, additional_points=None):
    df['timestamp'] = pd.to_datetime(df['timestamp'], format='mixed', dayfirst=True)

    house_id = os.path.splitext(os.path.basename(filepath))[0]

    # Ensure correct phase order (adjust as needed)
...
def create_changed_plots_from_path_for_seg_combined(filepath, original_cols, logger, additional_data):
    logger.info(f"Splitting data into segments for file: {filepath}")
    segments, points_by_segment = split_by_day_night_from_path(filepath, additional_data)

    logger.info(f"Number of segments found: {len(segments)}")
    check_or_create_house_directory(filepath)

    for i, (segment, points) in tqdm(enumerate(zip(segments, points_by_segment)), desc=f"Plotting segments for {os.path.basename(filepath)}"):
        logger.info(f"Plotting segment {i + 1} out of {len(segments)}")
        try:
            plot_combined_phases_interactive(segment, original_cols, filepath, additional_points=points)
            # plot_combined_phases(segment, original_cols, filepath, additional_points=points)
...
def process_visualization(house_id, run_number):
    ...
    additional_data = pd.read_csv(
        f"{seg_seq_directory}/on_off_1600.csv"
    )
