...
def fill_matches_in_data_if_uniform(data, on_event, matched_off, unmatched_on, off_events, event_power_col,
    ...
    if event_data.max() - event_data.min() > 500:
    ...
    try:
        ...
        logger.error(f"Error adjusting between {on_id} and {off_id} data ranges: {e}")
        return unmatched_on, off_events
...
def fill_matches_if_neighbors(data, on_event, matched_off, unmatched_on, off_events, event_power_col,
                              remaining_power, matches, diff_cal_name, logger):
    on_id = on_event['event_id']
    off_id = matched_off['event_id']
    phase = on_event['phase']
    on_magnitude = on_event['magnitude']
    off_magnitude = abs(matched_off['magnitude'])

    if (matched_off['start'] - on_event['end']).total_seconds() == 60:
        magnitude = min(on_magnitude, off_magnitude)

        try:
            prev_state = {
                "on_seg": data.loc[(data['timestamp'] >= on_event['start']) & (data['timestamp'] <= on_event['end']), [event_power_col, remaining_power]].copy(),
                "off_seg": data.loc[(data['timestamp'] >= matched_off['start']) & (data['timestamp'] <= matched_off['end']), [event_power_col, remaining_power]].copy()
            }

            on_range = (data['timestamp'] >= on_event['start']) & (data['timestamp'] <= on_event['end'])
            on_diff = data.loc[on_range, f"{phase}_diff"].cumsum()

            if on_magnitude > magnitude:
                data.loc[on_range, event_power_col] = magnitude + on_diff
                data.loc[on_range, remaining_power] = on_magnitude - (magnitude + on_diff)
            else:
                data.loc[on_range, event_power_col] = on_diff
                data.loc[on_range, remaining_power] = on_magnitude - on_diff

            off_range = (data['timestamp'] >= matched_off['start']) & (data['timestamp'] <= matched_off['end'])
            off_diff = data.loc[off_range, f"{phase}_diff"].cumsum()

            data.loc[off_range, event_power_col] = magnitude + off_diff
            data.loc[off_range, remaining_power] = off_magnitude - (magnitude + off_diff)

            data[diff_cal_name] = data[remaining_power].diff()

            if (data[event_power_col] < 0).any() or (data[remaining_power] < 0).any():
                logger.warning(f"Negative values detected between {on_id} and {off_id}, reverting changes.")
                data.loc[on_range, [event_power_col, remaining_power]] = prev_state["on_seg"]
                data.loc[off_range, [event_power_col, remaining_power]] = prev_state["off_seg"]
                unmatched_on.append(on_event)
                return unmatched_on, off_events
            ...
            logger.error(f"Error adjusting data of {on_id} and {off_id} for neighbors: {e}")
            unmatched_on.append(on_event)
            return unmatched_on, off_events
    else:
        unmatched_on.append(on_event)
...
def process_matches(house_id, run_number):
    ...
    for phase in tqdm(phases, desc=f"Processing phases for house {house_id}"):
        ...
        for _, on_event in tqdm(on_events.iterrows(), desc=f"Primary matching for phase {phase}", total=len(on_events)):
            potential_matches = off_events[
                ...
                ((off_events['start'] - on_event['end']) >= pd.Timedelta(0)) &
                ((off_events['start'] - on_event['end']) <= pd.Timedelta(minutes=2)) &
                (abs(off_events['magnitude'] + on_event['magnitude']) < 1000)
            ]
            ...
            if not potential_matches.empty:
                ...
                unmatched_on, off_events = fill_matches_if_neighbors(
        ...
        for _, on_event in tqdm(pd.DataFrame(unmatched_on).iterrows(), desc=f"Secondary matching for phase {phase}", total=len(unmatched_on)):
            potential_matches = unmatched_off[
                ...
                (abs(unmatched_off['magnitude'] + on_event['magnitude']) < 500)
            ]
    ...
    if matches_df.empty:
        logger.warning("No matches found. Skipping unmatched_on filtering.")

    else:
        all_unmatched_on = all_unmatched_on[
            ~all_unmatched_on['event_id'].isin(matches_df['on_event_id'])
        ]
    all_unmatched_on.drop_duplicates(inplace=True)

    matches_df.to_csv(f"{output_directory}/matches_{house_id}.csv", index=False)
