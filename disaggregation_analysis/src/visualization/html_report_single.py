"""
Single-house report section builders for disaggregation analysis.
...
def _generate_device_usage_html(device_usage: Dict[str, Any]) -> str:
    ...
    for device, data in device_usage.items():
        ...
        device_badge_map = {
            'central_ac': '<span style="background:#A8D8EA;color:#2D5F7C;padding:2px 8px;border-radius:4px;font-size:0.85em;">Central AC</span>',
            'regular_ac': '<span style="background:#B8E6C8;color:#2D6A3E;padding:2px 8px;border-radius:4px;font-size:0.85em;">Regular AC</span>',
            'boiler': '<span style="background:#FFCBA4;color:#7A4A1E;padding:2px 8px;border-radius:4px;font-size:0.85em;">Boiler</span>',
        }
        device_icon = device_badge_map.get(device, '<span style="background:#E8E4F0;color:#5D5D72;padding:2px 8px;border-radius:4px;font-size:0.85em;">Device</span>')
...
def _generate_flags_section(analysis: Dict[str, Any]) -> str:
    ...
    flag_config = {
        ...
        'has_recurring_patterns': {'icon': '[i]', 'color': '#155724', 'bg': '#d4edda', 'label': 'Recurring Patterns Detected'},
    }
...
def _generate_house_charts(analysis: Dict[str, Any]) -> str:
    ...
    if classification:
        ...
        device_counts = []
        device_colors = []
        color_map = {'boiler': '#dc3545', 'central_ac': '#007bff', 'regular_ac': '#28a745', 'recurring_pattern': '#17a2b8', 'unclassified': '#6c757d'}
        label_map = {'boiler': 'Boiler', 'central_ac': 'Central AC', 'regular_ac': 'Regular AC', 'recurring_pattern': 'Recurring Pattern', 'unclassified': 'Unclassified'}
